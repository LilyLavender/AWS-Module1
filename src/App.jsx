...
function App() {
  ...
  const apiEndpoint = "https://medals-lss-dqffe8fsgcfsh2bf.centralus-01.azurewebsites.net/jwtapi/country";
  const hubEndpoint = "https://medals-lss-dqffe8fsgcfsh2bf.centralus-01.azurewebsites.net/medalsHub";
  const userEndpoint = "https://medals-lss-dqffe8fsgcfsh2bf.centralus-01.azurewebsites.net/api/user/login";
  ...
  .sort((a, b) => b.gold.page_value - a.gold.page_value)
  .slice(0, 3)
  .map((c) => c.id);
  ...
  function sortCountries(a, b) {
    switch (sortMethod) {
      ...
      case "gold":
        return b.gold.page_value - a.gold.page_value;
      case "total":
        const totalA = medals.current.reduce((sum, medal) => sum + a[medal.name].page_value, 0);
        const totalB = medals.current.reduce((sum, medal) => sum + b[medal.name].page_value, 0);
  ...
  return (
    <Theme appearance={appearance}>
      ...
      <Flex p="2" pl="8" className="fixedHeader" justify="between">
        ...
        <Flex p="2" pl="8">
          <Select.Root value={sortMethod} onValueChange={setSortMethod}>
            <Select.Trigger placeholder="Sort by..." />
            <Select.Content>
              <Select.Item value="name">Name (A-Z)</Select.Item>
              <Select.Item value="gold">Gold Medals</Select.Item>
              <Select.Item value="total">Total Medals</Select.Item>
            </Select.Content>
          </Select.Root>
          {user.canPost && (
            <Tooltip content="Add country">
              <span><NewCountry onAdd={handleAdd} /></span>
            </Tooltip>
          )}
        </Flex>
      </Flex>
      ...
      <Grid pt="2" gap="2" className="grid-container">
        {countries
          ...
          .sort(sortCountries)
          .map((country) => {
            const goldRank = topGoldCountries.indexOf(country.id);
            return (
            <Country
              key={country.id}
              country={country}
